...
string HandleParsing(string[] request) {
    ...
    switch (request[0].ToLower()) {
        ...
        case "set":         
            if (request.Length > 4) {
                if (request.Length > 5 && request[3].ToLower() == "px") {
                    ...
                    Console.WriteLine($"Key: {request[1]}, Value: {request[2]}");
                    dict[request[1]] = new DataType { value = request[2], expiryTime = DateTime.Now.AddMilliseconds(100000) };
                    StartExpiryTask(request[1], 100000);
                    reply = "+OK\r\n";
                }
            }
            break;
